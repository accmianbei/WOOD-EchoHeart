...
- **用户偏好持久化**：回复面板和推特段子生成器都会记住常用设置，重新打开后可继续使用
- **恢复偏好入口**：支持一键恢复默认偏好，回复面板和推特生成器都已加入重置入口
- **推特生成器体验优化**：支持保存 RSS 源、关键词权重、情绪、长度和提示词草稿
- **稳定性修复**：修复部分页面在扩展重载后内容脚本重复注入导致的弹窗异常
...
> 提示：下载后需要先在 Chrome 扩展页开启「开发者模式」，否则无法加载和使用这个扩展。
...
- **Persistent preferences**: both the reply panel and the Tweet Bit Generator now remember user settings between sessions
- **Reset preferences actions**: both tools now include a one-click way to restore default preferences
- **Tweet generator improvements**: RSS sources, keyword weights, emotion, length, and prompt drafts can now be saved
- **Stability fix**: resolved a content script reinjection issue that could break the floating panel after extension reloads on some pages
...
> Note: After downloading, you need to enable **Developer mode** on the Chrome extensions page before this extension can be loaded and used.
